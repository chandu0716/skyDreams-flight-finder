...
mongoose.connect('mongodb://localhost:27017/FlightBookingMERN', { 
    ...
    app.post('/add-flight', async (req, res)=>{
        ...
        try{
            ...
            const flight = new Flight({flightName, flightId, origin, destination, 
                                        departureTime, arrivalTime, basePrice, totalSeats,createdBy});
    ...
    app.get('/fetch-my-flights/:id', async (req, res) => {
  const { id } = req.params;
  try {
    const flights = await Flight.find({ createdBy: id });
    res.json(flights);
  } catch (err) {
    console.log(err);
    res.status(500).json({ message: 'Failed to fetch operator flights' });
  }
});

    // ✅ NEW: search flights by origin and destination
